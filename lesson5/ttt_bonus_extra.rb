require 'pry'
#created to play with computer ai without modifying ttt_bonus.rb
...
class Board
  ...
  def two_in_a_row(target_marker)
    WINNING_LINES.each do |line|
      squares = @squares.values_at(*line)
      if two_identical_markers?(squares, target_marker) #-> line has a space we want
      # binding.pry
        target_space = line.select { |key| @squares[key].unmarked? }
...
class Computer < Player
  ...
  def middle_space_empty?
    board.unmarked_keys.include?(5)
  end

  def choose_middle_space
    board[5] = marker
  end
  ...
  def detect_winning_move
    !!board.two_in_a_row(marker)
  end

  def make_winning_move
    winning_square = board.two_in_a_row(marker)
    board[winning_square] = marker
  end

  def identify_human_marker
    marker == 'X' ? 'O' : 'X'
  end

  def detect_player_win
    !!board.two_in_a_row(identify_human_marker)
  end

  def block_player_win
    winning_square = board.two_in_a_row(identify_human_marker)
    board[winning_square] = marker
  end

  ## almost working -- sometimes randomly selecting instead of blocking player win
  def move
    if middle_space_empty?
      choose_middle_space
    elsif detect_winning_move  # make offenive move
      make_winning_move
    elsif detect_player_win
      block_player_win
    else
  ...
  # def move
  #   if middle_space_empty? #board.unmarked_keys.include?(5)
  #     choose_middle_space#board[5] = marker
  #   elsif board.missing_marker?
  #     board[board.square_with_missing_marker] = marker
  #   else
  #     select_random_square #board[board.unmarked_keys.sample] = marker
  #   end
  # end
end
...
class TTTGame
  ...
  def pick_names ## add display message with computer's name - option to expand to choose opponent
    human.pick_name
    computer.pick_name
    display_names
  end
